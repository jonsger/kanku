...
# Get kanku
[ -d /tmp/kanku ] || git clone https://github.com/M0ses/kanku.git || exit 1

# Install kanku
make -C /tmp/kanku install || exit 1

# create kanku user
adduser $KANKU_USER sudo || exit 1
adduser $KANKU_USER libvirt || exit 1
cp /tmp/kanku/dist/sudoers-kanku-setup /etc/sudoers.d/kanku-setup || exit 1

# needed on ubuntu
ln -s /usr/bin/kvm /usr/bin/qemu-kvm || exit 1
